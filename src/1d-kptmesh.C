/*
  Program: 1d-kptmesh.C
  Author:  D. Trinkle
  Date:    2012 July 19
  Purpose: Generate a kptmesh along 1d lines in the BZ (for planar LGF calc)

  Param.:  cell plane N
           cell:     cell file (see below for format)
           plane:    input file (see below for format)
	   N:        divisions in the BZ direction
	   ...
	   ==== plane ====
	   t1 t2 t3     # planar threading direction
	   t1 t2 t3     # second planar threading direction
	   n1 n2 n3     # perpendicular vector (t1 x t2)
	   ==== plane ====
...
#include "cell.H"
#include "integrate.H"
#include "plane.H" // included to use make_unit, e.g.
#include "kpts.H"
#include "fourier.H"  // included just to get our hands on the gcd algorithm
...
const int NUMARGS = 3;
const char* ARGLIST = "cell plane N";
...
int main ( int argc, char **argv ) 
{
  ...
  int N; sscanf(args[2], "%d", &N);
  int T1, T2; // number of divisions for t directions; calculated later.
  ...
  if (ERROR) {
    fprintf(stderr, "Problem with plane input file: %s\n", plane_name);
    exit(ERROR);
  }
  ...
  int plane[9], plane_inv[9], plane_det;
  ...
  for (d=0; d<3; ++d) {
    plane[3*d+0] = t1_unit[d];
    plane[3*d+1] = t2_unit[d];
    plane[3*d+2] = n_unit[d];
  }
  plane_det = inverse(plane, plane_inv);
  self_transpose(plane_inv);
  // Now, we need to determine our "multipliers":
  int scale; // the last one will be the gcd for the t direction
  for (i=0; i<3; ++i) {
    scale = gcd (plane_inv[i], plane_inv[i+3], plane_inv[i+6]);
    for (d=0; d<3; ++d) plane_inv[i+3*d] /= scale;
    if (i==0) T1 = plane_det/scale;
    if (i==1) T2 = plane_det/scale;
  }
  ...
  if (TESTING) {
    ...
    double kptcell[9];
    mult(cart_rlv, plane_inv, kptcell);
    for (i=0; i<3; ++i) {
      printf("## k%d:", i+1);
      for (d=0; d<3; ++d) printf(" %3d", plane_inv[i+3*d]);
      printf(" | ");
  ...
  for (ni[0]=-Ni[0]+1; ni[0] <= Ni[0]; ++(ni[0])) {
    ...
    for (ni[1]=-Ni[1]+1; ni[1] <= Ni[1]; ++(ni[1])) {
      ...
      for (ni[2]=-Ni[2]+1; ni[2] <= Ni[2]; ++(ni[2])) {
        q0[2] = dN[2] * (ni[2]+dq);
	mult_vect(plane_inv, q0, q); // added to put us on them planes
  ...
  printf("%d 1.0 L # Nkpt, scale, lattice coord: %d mesh along [%d %d %d] x [%d %d %d]\n",
         Nkpt, N, 
