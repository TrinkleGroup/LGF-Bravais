...
inline double Rmin2 (const double cart[9]);

inline void min_u (const double cart[9], double R[3], double u[3]);
...
int main ( int argc, char **argv ) 
{
  ...
  double cart_inv[9], du[3], R[3];
  int u[3];
  double umin2, utotal[3] = {0,0,0};
  ...
  for (np=0; np<Np; ++np) {
    ...
    mult_vect(cart_inv, tp->R, du);
    for (d=0; d<3; ++d) u[d] = lround(du[d]);
    mult_vect(cart, u, R);
    for (d=0; d<3; ++d) tp->u[d] = tp->R[d] - R[d];
    for (d=0; d<3; ++d) tp->R[d] = R[d];

    if (dot(tp->u, tp->u) > umin2) min_u(cart, tp->R, tp->u);
...
{
  double umin2;
  double v[3], unew[3], vbest[3] = {0,0,0};
  int n[3];
  ...
  for (n[0]=-1; n[0]<=1; ++(n[0]))
    for (n[1]=-1; n[1]<=1; ++(n[1]))
      for (n[2]=-1; n[2]<=1; ++(n[2])) {
	mult_vect(cart, n, v);
	for (int d=0; d<3; ++d) unew[d] = u[d]+v[d];
	double R2 = dot(unew,unew);
	if (R2 < umin2) {
	  umin2 = R2;
	  for (int d=0; d<3; ++d) vbest[d] = v[d];
	}
      }
  for (int d=0; d<3; ++d) {
    u[d] += vbest[d];
