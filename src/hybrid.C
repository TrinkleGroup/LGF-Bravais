/*
  ...
  Purpose: This takes in:
	   ...
	   4. the discretization correction for the elastic GF
	   ...
	   method.  It's my first real attempt, and will probably fail
	   miserably (as in, produce terrible errors in the phonons).

  Param.:  <cell> <supercell> <folded Dij> <disc.elas>
...
const int NUMARGS = 4;
const char* ARGLIST = "<cell> <supercell> <folded Dij> <disc.elas>";

const int NFLAGS = 1;
const char USERFLAGLIST[NFLAGS] = {'f'}; // determine how we handle k=0
// flag characters.
...
"  cell:       cell file describing our lattice\n\
  ...
  folded Dij: folded down dynamical matrix (i.e., direct force calc)\n\
  disc.elas:  discretization correction of EGF\n\
  -f          fix the furthest R point to have the EGF (sets k=0 value)";
...
int main ( int argc, char **argv ) 
{
  ...
  // flags
  int FIXED = flagon[0]; // use fixed boundaries

  if (TESTING) {
    if (FIXED) 
      printf("## Setting k=0 value to make deltaG(Rmax) = 0\n");
    else 
      printf("## Setting k=0 according to average lambda2\n");
  }
  ...
  char *cell_name, *super_name, *latt_name, *elas_name;
  ...
  // 3. Construct our (approximate) lambda2 matrix from our folded
  ...
  for (k=0; k<Nkpt; ++k) {
    ...
    if (! zero(tkpt[3])) {
      double Dk[9], Dkinv[9], kCk[9], GEk[9];
      // a. D(k):
      ...
      mult(GEk, kpoint_envelope(tkpt[3]/kmax), GEk);
      for (d=0; d<9; ++d) dGk[k][d] = Dkinv[d] - GEk[d];
    }
    else {
      kgamma = k;
      // set the gamma value based on lambda2:
      if (! FIXED) avg_4pi(Cijkl, lambda2, dGk[k]);
      else         for (d=0; d<9; ++d) dGk[k][d] = 0.;
    }
    // GE.d(k):
    double GEdk[9];
  ...
  // 4.b. handle "fixed" boundaries--make the point Nsuper-1 (the 
  //      furthest point out in our supercell--have dGR = 0.
  if (FIXED) {
    double sum[9];
    for (d=0; d<9; ++d) sum[d] = 0.;
    point_type *tp = dGR + (Nsuper-1);
    for (k=0; k<Nkpt; ++k) {
      double *tkpt = klist_cart[k];
      double coskR = cos(dot(tp->Rcart, tkpt));
      for (d=0; d<9; ++d) sum[d] += w[k]*coskR*dGk[k][d];
    }
    // Now, set dGk[gamma] = -sum / w(gamma)
    mult(sum, -1./w[kgamma], dGk[kgamma]);
  }
  ...
  // 7. Finally, we combine the GEd with dGR.
  sort_points(GEd, Nd);
